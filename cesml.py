...
class Linky():
    def __init__(self,username,password,client_id,proxy={}):
    ...
    def loginCESML(self):
      ...
      response = self.session.post(url, headers=headers, data=payload,proxies=self.proxy,verify=False)
      ...
      response = self.session.get( url, headers={}, data={},proxies=self.proxy,verify=False,hooks=dict(response=self.get_accesstoken))
      return token
    ...
    def getMesures(self,dateDebut=datetime.datetime(2023,11,16,0,0,0,0)):
      ...
      response = s.post(url, headers=headers, data=payload,proxies=self.proxy,verify=False)
